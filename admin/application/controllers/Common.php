...
class Common extends MY_Controller {
    ...
    public function TeamMembers() {
		...
		$this->session->set_userdata(array('back_url' => base_url($this->uri->uri_string())));
    ...
    public function SalesClients(){
		...
		$role = $this->uri->segment(4); // 4: Sales Cosult 5 : Case Manager
		$this->load->model('Client_model');
		$userName				=   $this->Defualt_Model->findSelect('users', array('name'), array('id' => $id)); 
		$data['page_name'] 		= 	$userName[0]->name.' Clients';
		$data['clients'] 		= 	$this->Client_model->allClientsWithDetails($id, $role);
		$this->load->view('Common/sales_clients', $data);
	}

	public function DocsClients(){
		$id = $this->uri->segment(3);
		$role = $this->uri->segment(4); // 4: Sales Cosult 5 : Case Manager
		$this->load->model('Client_model');
		$userName				=   $this->Defualt_Model->findSelect('users', array('name'), array('id' => $id)); 
		$data['page_name'] 		= 	$userName[0]->name.' Clients';
		$data['clients'] 		= 	$this->Client_model->allClientsWithDetails($id, $role);
		$this->load->view('Common/docs_clients', $data);
	}
