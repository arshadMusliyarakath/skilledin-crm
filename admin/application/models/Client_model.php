...
class Client_model extends CI_Model {
    ...
    public function allClientsWithDetails($added_by, $role) {
        $this->db->select('clients.*, products.product_name');
        $this->db->from('clients');
        ($role == 4) ? $this->db->where('clients.added_by', $added_by) : $this->db->where('clients.case_manager', $added_by) ;
    ...
    public function waitingClients(){
        $this->db->select('cl.id, cl.name, us.name as consultant_name, pr.product_name');
    ...
    public function allClientsWithConsultent() {
        $this->db->select('clients.*, products.product_name, users.name as consultent_name, us.name as case_manager');
        $this->db->from('clients');
        $this->db->join('products', 'products.id = clients.product', 'left');
        $this->db->join('users', 'users.id = clients.added_by', 'left');
        $this->db->join('users as us', 'us.id = clients.case_manager', 'left');
        $this->db->order_by('clients.id', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
